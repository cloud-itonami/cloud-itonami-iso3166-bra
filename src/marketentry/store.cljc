(ns marketentry.store
  "SSoT for the BRA market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems BR" :portal "Compras.gov.br"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-br-entity? true :has-br-entity? true
             :requires-cnpj? true :cnpj-verified? true
             :drafted? false :submitted? false
             :jurisdiction "BRA" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "Compras.gov.br"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-br-entity? true :has-br-entity? true
             :requires-cnpj? true :cnpj-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "Compras.gov.br"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "BRA" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "Compras.gov.br"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-br-entity? true :has-br-entity? false
             :requires-cnpj? true :cnpj-verified? true
             :drafted? false :submitted? false
             :jurisdiction "BRA" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "Compras.gov.br"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-br-entity? true :has-br-entity? true
             :requires-cnpj? true :cnpj-verified? false
             :drafted? false :submitted? false
             :jurisdiction "BRA" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "Compras.gov.br"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-br-entity? has-br-entity?
                               requires-cnpj? cnpj-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-br-entity?)  (assoc :engagement/requires-br-entity? requires-br-entity?)
    (some? has-br-entity?)       (assoc :engagement/has-br-entity? has-br-entity?)
    (some? requires-cnpj?)    (assoc :engagement/requires-cnpj? requires-cnpj?)
    (some? cnpj-verified?)    (assoc :engagement/cnpj-verified? cnpj-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-br-entity? :engagement/has-br-entity?
   :engagement/requires-cnpj? :engagement/cnpj-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-br-entity? (boolean (:engagement/requires-br-entity? m))
     :has-br-entity? (boolean (:engagement/has-br-entity? m))
     :requires-cnpj? (boolean (:engagement/requires-cnpj? m))
     :cnpj-verified? (boolean (:engagement/cnpj-verified? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-bra"})
