(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Brazil-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Brazil-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-br-entity?
                                       true` (near-universal for Brazil
                                       ...
                                       Grounded in Brazilian legal entity
                                       (CNPJ) typically required for federal
                                       procurement participation (contracting
                                       authorities / Compras.gov.br).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(CNPJ/統一資格/Compras.gov.br登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- br-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-br-entity? true`, INDEPENDENTLY verify
  `:has-br-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-br-entity?` ground truth (most Brazil public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-br-entity? e))
                 (not (true? (:has-br-entity? e))))
        [{:rule :br-entity-missing
          :detail (str subject " はブラジル居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- cnpj-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-cnpj? true`, INDEPENDENTLY check
  `:cnpj-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-cnpj? e))
                 (not (true? (:cnpj-verified? e))))
        [{:rule :cnpj-unverified
          :detail (str subject " はCNPJ確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (br-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (cnpj-unverified-violations request st)
