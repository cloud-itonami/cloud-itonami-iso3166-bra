(ns marketentry.marketentryllm
  ...
  the BRA public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-br-entity?=" (:has-br-entity? e)
                        " cnpj-verified?=" (:cnpj-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-br-entity? e))
                              (:has-br-entity? e))
                          (or (not (:requires-cnpj? e))
                              (:cnpj-verified? e)))
                   0.9 0.3)}))
