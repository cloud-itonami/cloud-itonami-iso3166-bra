# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Brazil

## Classification

- Repository: `cloud-itonami-iso3166-bra`
- ISO 3166: `BRA` (Brazil)
...
- Social impact: [:sme-preferential-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on a Brazilian
  public contract
...
- registration walkthrough for the ComprasNet / Portal Nacional de Contratações Públicas (PNCP) federal e-procurement portal, governed by the Public Procurement Law (Lei de Licitações e Contratos, Lei 14.133/2021)
- business/tax registration checklist: a CNPJ (Cadastro Nacional da Pessoa Jurídica) plus SICAF (Sistema de Cadastramento Unificado de Fornecedores) supplier registration; state-level ICMS and municipal ISS tax registration where applicable, alongside federal tax obligations
- local-content / preferential-procurement navigation: ME/EPP (micro and small enterprise) preferential-bidding margins under the Lei Geral da Micro e Pequena Empresa, and local-content requirements on certain works/manufacturing tenders
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Brazilian-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Brazil).
